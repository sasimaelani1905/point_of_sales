...
// -------------------------------------------------------------
// 1. MENGAMBIL DATA METRIK UTAMA (PENJUALAN, PEMBELIAN, LABA, BARANG)
// -------------------------------------------------------------

// Total Penjualan (Nominal Rp dari tbl_nota_jual)
...
// Total Pembelian (Nominal Rp dari tbl_nota_beli)
...
// Estimasi Laba Bersih (Penjualan - Pembelian)
...
// -------------------------------------------------------------
// 2. DATA UNTUK DIAGRAM TOP 5 PEMBELIAN BARANG TERTINGGI (DARI NOTA BELI)
// -------------------------------------------------------------
$array_nama_brg_beli = [];
$array_jumlah_beli   = [];

$q_top_pembelian = mysqli_query($koneksi, "
    SELECT kode_brg, SUM(jumlah) AS total_jumlah 
    FROM tbl_detail_nota_beli 
    GROUP BY kode_brg 
    ORDER BY total_jumlah DESC 
    LIMIT 5
");

if ($q_top_pembelian && mysqli_num_rows($q_top_pembelian) > 0) {
    while ($row = mysqli_fetch_assoc($q_top_pembelian)) {
        $kd_brg = $row['kode_brg'];
        $jumlah = $row['total_jumlah'];

        // Query nama barang terpisah tanpa JOIN
        ...
        $array_nama_brg_beli[] = $nama_brg;
        $array_jumlah_beli[]   = (int)$jumlah;
    }
} else {
    $array_nama_brg_beli = ['Belum Ada Data'];
    $array_jumlah_beli   = [0];
}

$json_nama_brg_beli = json_encode($array_nama_brg_beli);
$json_jumlah_beli   = json_encode($array_jumlah_beli);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <!-- Small boxes (Stat box - Sama Persis Gambar AdminLTE 3 v1) -->
        <div class="row">
          <!-- Box 1: Total Penjualan (Biru) -->
          ...
          <!-- Box 3: Estimasi Laba Bersih (Kuning) -->
          <div class="col-lg-3 col-6">
        ...
        <!-- Main row (Charts Section) -->
        <div class="row">
          <!-- Diagram Kiri: Ringkasan Penjualan, Pembelian, & Laba -->
          <section class="col-lg-7 connectedSortable">
            <div class="card">
              <div class="card-header border-0">
                <h3 class="card-title">
                  <i class="fas fa-chart-pie mr-1"></i>
                  Statistik Penjualan, Pembelian & Laba
                </h3>
              </div>
              <div class="card-body">
          ...
          <!-- Diagram Kanan: Top 5 Pembelian Barang Tertinggi -->
          <section class="col-lg-5 connectedSortable">
            <div class="card">
              <div class="card-header border-0">
                <h3 class="card-title">
                  <i class="fas fa-th mr-1"></i>
                  Top 5 Pembelian Barang Tertinggi
                </h3>
              </div>
              <div class="card-body">
                <canvas id="topPembelianChart" style="min-height: 300px; height: 300px; max-height: 300px; max-width: 100%;"></canvas>
              </div>
            </div>
          </section>
        </div>
...
<script>
  $(document).ready(function () {
    // 1. Chart Statistik Keuangan (Penjualan, Pembelian, Laba)
    ...
    // 2. Chart Top 5 Pembelian Barang Tertinggi
    var namaBarangBeli = <?= $json_nama_brg_beli; ?>;
    var jumlahBeli     = <?= $json_jumlah_beli; ?>;

    var ctxBeli = document.getElementById('topPembelianChart').getContext('2d');
    new Chart(ctxBeli, {
      type: 'horizontalBar',
      data: {
        labels: namaBarangBeli,
        datasets: [{
          label: 'Jumlah Dibeli (Pcs)',
          data: jumlahBeli,
